package de.kxmischesdomi.mushroom.feature;
 ...
 * @since 1.0
 */
public class HugeGlowMushroomFeature extends AbstractHugeMushroomFeature {

	public HugeGlowMushroomFeature(Codec<HugeMushroomFeatureConfiguration> codec) {
		super(codec);
	}
				...
				for (int n = -k; n <= k; ++n) {
					...
					if (blockState.hasProperty(HugeMushroomBlock.WEST) && blockState.hasProperty(HugeMushroomBlock.EAST) && blockState.hasProperty(HugeMushroomBlock.NORTH) && blockState.hasProperty(HugeMushroomBlock.SOUTH) && blockState.hasProperty(HugeMushroomBlock.UP)) {
						blockState = blockState.
								setValue(HugeMushroomBlock.UP, j >= i - 1)
